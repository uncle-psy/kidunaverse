import { coherenceDimensions, evaluateTransmissionDraft, fieldObjects, historyEntries, layoutTypes, packages, receiverThemes, receiverV2Catalog, receiverV2ReceivedAt, transmitterCatalog, transmitterDestinations, transmitterSeedCatalog, transmitterSeedTypes, transmitterSenders } from './data.js?v=0.6.0';
...
function receiverV2Inspector(state, profile) {
  ...
  const rows = [
    ['Stable ID', item.id], ['Object class', item.kind], ['Received', new Date(receiverV2ReceivedAt[item.id]).toLocaleString()], ['Source / originator', item.origin], ['Transmitting context', item.eyebrow],
