...
const visibleObjectClass = kind => kind === 'Package' ? 'Seed' : kind === 'Power Map' ? 'Map' : kind;

export function railAction(item, active) {
  const binding = item.placeholder ? `data-placeholder="${item.id}" aria-label="${item.label} — placeholder"` : `data-panel="${item.id}" aria-label="${item.label}" aria-pressed="${active}"`;
  return `<button class="rail-action ${active ? 'active' : ''} ${item.placeholder ? 'placeholder' : ''}" type="button" ${binding}>
    <span class="rail-role-icon role-icon-${item.roleIcon}" aria-hidden="true"></span>
    <span class="rail-tooltip"><b>${item.label}</b>${item.placeholder ? '<small>Placeholder</small>' : ''}</span>
  </button>`;
}
...
function worldWorld() {
  return `<div class="world-layout mode-world" aria-label="World Layout Type — full 3D environment"><div class="world-sky" aria-hidden="true"></div><div class="world-ground" aria-hidden="true"><i></i><i></i><i></i></div><div class="world-portal one"><span>Portal</span><b>Commons Forum</b></div><div class="world-portal two"><span>Realm</span><b>Shared Garden</b></div><div class="world-presence"><span>M</span><b>Moto</b><small>Avatar · present in Rehearsal</small></div><div class="world-copy"><span>World · full 3D</span><h2>The Future Commons</h2><p>A spatial environment for movement, embodied presence, simulation, and authorized action.</p></div><div class="mode-caption"><span>World</span><strong>Embodied environment</strong><small>3D · Rehearsal · prototype view</small></div></div>`;
}

function readerWorld() {
  return `<div class="reader-layout mode-world" aria-label="Reader Layout Type — document and editorial experience"><article class="reader-paper"><header><span>Field Notes · Issue 01</span><h1>Invitation as Orientation</h1><p class="reader-deck">What changes when an invitation reveals living work before it asks for commitment?</p></header><div class="reader-byline">By Moto · Personal working record · Sep 3, 2026</div><div class="reader-columns"><p><b>An invitation can be a map.</b> It can show where people are, what is already moving, and why a particular contribution might matter—without deciding for the person being invited.</p><blockquote>Belonging begins with enough context to choose freely.</blockquote><p>This Reader joins notes, a Kinship Map, a short audio annotation, and the visible provenance of Alice’s invitation.</p><aside><span>Audio annotation · 2:14</span><button type="button" data-prototype-action="Play audio annotation">▶ Listen</button></aside></div></article><div class="mode-caption"><span>Reader</span><strong>Documents and editorial Media</strong><small>Reading-led · audio and video supported</small></div></div>`;
}

function gameWorld() {
  return `<div class="game-layout mode-world" aria-label="Game Layout Type — playable rules and feedback"><div class="game-board"><header><span>Game · Rehearsal</span><h1>Royals &amp; Rogues</h1><p>Choose a House, make an Offer, and shift the balance before the turn closes.</p></header><div class="game-score"><article><span>Moto</span><strong>12</strong><small>Influence</small></article><article><span>Alice</span><strong>9</strong><small>Trust</small></article><article><span>Commons</span><strong>18</strong><small>Stability</small></article></div><div class="game-path" aria-label="Playable path"><button data-prototype-action="Choose the Builder House"><b>Builder</b><small>+2 stewardship</small></button><i>→</i><button data-prototype-action="Offer an alliance"><b>Alliance</b><small>Requires consent</small></button><i>→</i><button data-prototype-action="Resolve the current turn"><b>Turn 4</b><small>2 choices remain</small></button></div><footer><button data-prototype-action="Inspect game rules">Rules</button><button data-prototype-action="Save game rehearsal">Save Rehearsal</button><button class="primary-button" data-prototype-action="Preview next game turn">Preview next turn</button></footer></div><div class="mode-caption"><span>Game</span><strong>Rules, choice, and feedback</strong><small>Playable · bounded Rehearsal state</small></div></div>`;
}
...
export function fieldStage(state) {
  ...
  if (state.type === 'world') return worldWorld();
  if (state.type === 'reader') return readerWorld();
  if (state.type === 'game') return gameWorld();
...
function receiverV2Inspector(state, profile) {
  ...
  const rows = [
    ['Stable ID', item.id], ['Object class', visibleObjectClass(item.kind)], ['Received', new Date(receiverV2ReceivedAt[item.id]).toLocaleString()], ['Source / originator', item.origin], ['Transmitting context', item.eyebrow],
...
function receiverV2ProfileManager(state, active) {
  ...
  return `<section class="receiver-v2-profile-manager" aria-label="Manage Reception Profiles">
    <header><div><span>Receiver configuration</span><h2>Reception Profiles</h2><p>Profiles preserve distinct, reusable ways of tuning reception.</p></div><button type="button" data-action="receiver-v2-close-profiles" aria-label="Close profile manager">${icon('close', 20)}</button></header>
...
function receivePanelV2(state) {
  ...
  let items = receiverV2Catalog.filter(item => (!query || searchable(item).includes(query)) && filters.has(item.kind) && (!item.sponsored || filters.has('Sponsored')));
  items = [...items].sort((a, b) => {
    const aTune = receiverV2Tuning(profile, a); const bTune = receiverV2Tuning(profile, b);
    if (state.receiverV2Sort === 'Most recent') return new Date(receiverV2ReceivedAt[b.id]) - new Date(receiverV2ReceivedAt[a.id]) || a.title.localeCompare(b.title);
    if (state.receiverV2Sort === 'Oldest') return new Date(receiverV2ReceivedAt[a.id]) - new Date(receiverV2ReceivedAt[b.id]) || a.title.localeCompare(b.title);
  ...
  const classes = ['Transmitter', 'Realm', 'Power Map', 'Package', 'Node', 'Sponsored'];
  const allSelected = classes.every(item => filters.has(item));
  const receiverEyebrow = receiverEyebrows[Math.max(0, state.receiverEyebrowIndex) % receiverEyebrows.length];
  return `<section class="receiver-v2" aria-label="Receiver Version 0.02" data-open-panel="receive">
    ...
    <header class="receiver-v2-topbar"><div class="receiver-v2-brand"><span class="rail-role-icon role-icon-receiver" aria-hidden="true"></span><div><span>${escapeHtml(receiverEyebrow)}</span><h1>Receiver</h1></div></div><label class="receiver-v2-profile"><span>Reception Profile</span><select id="receiver-v2-profile-select">${state.receiverProfiles.map(item => `<option value="${item.id}" ${item.id === profile.id ? 'selected' : ''}>${escapeHtml(item.name)}</option>`).join('')}</select></label><button class="receiver-v2-manage" type="button" data-action="receiver-v2-manage-profiles">Manage profiles</button><button class="receiver-v2-close" type="button" data-close-panel aria-label="Close Receiver">${icon('close', 22)}</button></header>
    <section class="receiver-v2-controls"><label class="receiver-v2-search">${icon('search', 19)}<span class="sr-only">Search reception field</span><input id="receiver-v2-search" type="search" value="${escapeHtml(state.receiverV2Query)}" placeholder="Search transmitters, Realms, Maps, Seeds, Nodes, topics, Vibes…"></label><nav aria-label="Receiver visibility filters"><button class="${allSelected ? 'active' : ''}" type="button" data-v2-filter="All" aria-pressed="${allSelected}">All signals</button>${classes.map(item => { const selected = filters.has(item); const label = item === 'Power Map' ? 'Maps' : item === 'Package' ? 'Seeds' : item === 'Sponsored' ? 'Sponsored' : `${item}s`; return `<button class="${selected ? 'active' : ''}" type="button" data-v2-filter="${item}" aria-pressed="${selected}">${label}</button>`; }).join('')}</nav><label class="receiver-v2-sort"><span>Sort by</span><select id="receiver-v2-sort"><option ${state.receiverV2Sort === 'Most recent' ? 'selected' : ''}>Most recent</option><option ${state.receiverV2Sort === 'Oldest' ? 'selected' : ''}>Oldest</option><option ${state.receiverV2Sort === 'Intensity' ? 'selected' : ''}>Intensity</option><option ${state.receiverV2Sort === 'Range' ? 'selected' : ''}>Range</option><option ${state.receiverV2Sort === 'Topic' ? 'selected' : ''}>Topic</option><option ${state.receiverV2Sort === 'Transmitter' ? 'selected' : ''}>Transmitter</option></select></label><button class="receiver-v2-recommend-button" type="button" data-action="receiver-v2-show-recommendations">Ki recommends</button>${state.receiverUndo ? '<button class="receiver-v2-undo" type="button" data-action="receiver-undo">Undo</button>' : ''}</section>
    <div class="receiver-v2-scroll"><div class="receiver-v2-field-heading"><div><span>${visible.length} of ${items.length} visible</span><h2>Tune the Field</h2><p>Intensity controls how much of the exact object reaches this profile. Range explores visibility-safe relationships. Neither changes shared Field truth.</p></div><div><span>Current sort</span><strong>${state.receiverV2Sort}</strong></div></div>${receiverV2Recommendations(state, profile)}<div class="receiver-v2-grid">${visible.map(item => receiverV2Box(state, profile, item)).join('') || '<div class="receiver-v2-empty"><h2>No visible matches</h2><p>Select at least one visible signal class or adjust the search. Private exclusions are not counted or disclosed.</p></div>'}</div>${visible.length < items.length ? `<button class="receiver-v2-more" type="button" data-action="receiver-v2-more">Show ${Math.min(6, items.length - visible.length)} more</button>` : ''}</div>
    ${receiverV2ProfileManager(state, profile)}${receiverV2Inspector(state, profile)}
  </section>`;
}
...
const privacyOrder = { Public: 0, Private: 1, Secret: 2, Personal: 3 };

function transmitterDraft(state) {
  return { seedIds: state.transmitterV2SeedIds, destinationIds: state.transmitterV2DestinationIds, senderId: state.transmitterV2SenderId, message: state.transmitterV2Message, messagePrivacy: state.transmitterV2MessagePrivacy };
}

function transmitterSeedCard(state, seed) {
  const selected = state.transmitterV2SeedIds.includes(seed.id);
  return `<article class="tx-seed-card ${selected ? 'selected' : ''}" data-tx-seed-card="${seed.id}"><div class="tx-seed-visual">${seed.image ? `<img src="${seed.image}" alt="">` : `<span>${escapeHtml(seed.type.slice(0, 2))}</span>`}<i>${escapeHtml(seed.privacy)}</i></div><div class="tx-seed-copy"><span>${escapeHtml(seed.eyebrow)}</span><h3>${escapeHtml(seed.title)}</h3><p>${escapeHtml(seed.description)}</p><small>${escapeHtml(seed.type)} · ${escapeHtml(seed.creator)} · ${escapeHtml(seed.status)}</small></div><div class="tx-seed-actions"><button type="button" data-tx-inspect="${seed.id}">Inspect</button><button class="${selected ? 'remove' : 'add'}" type="button" ${selected ? `data-tx-seed-remove="${seed.id}"` : `data-tx-seed-add="${seed.id}"`}>${selected ? 'Remove' : 'Add to Payload'}</button></div></article>`;
}

function transmitterPrivacyCheck(result) {
  const tone = result.ready ? 'ready' : result.issues.some(issue => issue.kind === 'privacy' || issue.kind === 'sender') ? 'blocked' : 'incomplete';
  return `<section class="tx-privacy-check ${tone}" aria-live="polite"><div><span>Live privacy check</span><strong>${escapeHtml(result.status)}</strong></div>${result.destinationResults.length ? `<ul>${result.destinationResults.map(({ destination, issues }) => `<li><i></i><span><b>${escapeHtml(destination.name)}</b><small>${issues.length ? escapeHtml(issues.map(issue => issue.detail).join(' ')) : `Compatible · ${destination.privacy} boundary`}</small></span></li>`).join('')}</ul>` : '<p>Choose a destination to compare every Message and Seed boundary.</p>'}</section>`;
}

function transmitterBuilder(state, result) {
  const seeds = state.transmitterV2SeedIds.map(id => transmitterSeedCatalog.find(item => item.id === id)).filter(Boolean);
  const sender = transmitterSenders.find(item => item.id === state.transmitterV2SenderId) || transmitterSenders[0];
  const destinationQuery = state.transmitterV2DestinationQuery.toLowerCase();
  const destinations = transmitterDestinations.filter(item => !destinationQuery || [item.name, item.type, item.context, item.privacy, item.retention].join(' ').toLowerCase().includes(destinationQuery));
  return `<aside class="tx-builder" aria-label="Signal builder"><header><div><span>Signal builder</span><h2>Message + Payload</h2></div><strong>${seeds.length} Seed${seeds.length === 1 ? '' : 's'}</strong></header>
    <section class="tx-builder-section tx-message"><div class="tx-section-title"><span>01 · Message</span><small>What travels with your stuff</small></div><textarea id="tx-message" aria-label="Signal Message" rows="4" placeholder="Write the Message that accompanies this Payload…">${escapeHtml(state.transmitterV2Message)}</textarea><label>Message privacy<select id="tx-message-privacy">${Object.keys(privacyOrder).map(level => `<option ${state.transmitterV2MessagePrivacy === level ? 'selected' : ''}>${level}</option>`).join('')}</select></label></section>
    <section class="tx-builder-section tx-payload"><div class="tx-section-title"><span>02 · Payload</span><small>One or more individually identified Seeds</small></div><input id="tx-payload-name" value="${escapeHtml(state.transmitterV2PayloadName)}" aria-label="Payload name" placeholder="Name this Payload"><ol>${seeds.map((seed, index) => `<li><span><b>${escapeHtml(seed.title)}</b><small>${seed.type} · ${seed.privacy}</small></span><div><button type="button" data-tx-seed-up="${seed.id}" ${index === 0 ? 'disabled' : ''} aria-label="Move ${escapeHtml(seed.title)} up">↑</button><button type="button" data-tx-seed-down="${seed.id}" ${index === seeds.length - 1 ? 'disabled' : ''} aria-label="Move ${escapeHtml(seed.title)} down">↓</button><button type="button" data-tx-seed-remove="${seed.id}" aria-label="Remove ${escapeHtml(seed.title)}">×</button></div></li>`).join('') || '<li class="empty">Add at least one Seed from your library.</li>'}</ol><div class="tx-payload-actions"><button type="button" data-action="tx-clear-payload">Clear</button><button type="button" data-action="tx-save-draft">Save Draft</button><button type="button" data-action="tx-inspect-payload">Inspect</button></div></section>
    <section class="tx-builder-section tx-sender-section"><div class="tx-section-title"><span>03 · Send as</span><small>What recipients will see</small></div><select id="tx-sender">${transmitterSenders.map(item => `<option value="${item.id}" ${item.id === sender.id ? 'selected' : ''}>${escapeHtml(item.name)} · ${item.type}</option>`).join('')}</select><p class="tx-recipient-view"><b>Recipient sees:</b> ${escapeHtml(sender.recipientView)}<br><small>${escapeHtml(sender.authority)}</small></p></section>
    <section class="tx-builder-section tx-destinations"><div class="tx-section-title"><span>04 · Destinations</span><small>Choose one or more receiving contexts</small></div><input id="tx-destination-search" type="search" aria-label="Search destinations" value="${escapeHtml(state.transmitterV2DestinationQuery)}" placeholder="Search Realms, Receivers, Sources, Actors, Allies…"><div>${destinations.map(destination => { const selected = state.transmitterV2DestinationIds.includes(destination.id); return `<article><button class="${selected ? 'selected' : ''}" type="button" data-tx-destination="${destination.id}" aria-pressed="${selected}"><span>${escapeHtml(destination.type)}</span><b>${escapeHtml(destination.name)}</b><small>${escapeHtml(destination.privacy)} · ${escapeHtml(destination.context)}</small></button><button type="button" data-tx-inspect-destination="${destination.id}" aria-label="Inspect ${escapeHtml(destination.name)}">i</button></article>`; }).join('') || '<p>No destinations match this search.</p>'}</div></section>
    ${transmitterPrivacyCheck(result)}<button class="tx-review-button" type="button" data-action="tx-review">Review Signal</button></aside>`;
}

function transmitterInspector(state) {
  const seed = transmitterSeedCatalog.find(item => item.id === state.transmitterV2InspectorId);
  const destination = transmitterDestinations.find(item => item.id === state.transmitterV2DestinationInspectorId);
  if (!seed && !destination) return '';
  if (destination) {
    const rows = [['Stable ID', destination.id], ['Destination class', destination.type], ['Privacy boundary', destination.privacy], ['Context', destination.context], ['Retention', destination.retention], ['Anonymous presentation', destination.anonymousAllowed ? 'Permitted' : 'Not permitted'], ['Secret handling', destination.secretHandling ? 'Declared' : 'Not declared'], ['Allowed transmitting identities', destination.allowedSenders.map(id => transmitterSenders.find(sender => sender.id === id)?.name || id).join(', ')]];
    return `<aside class="tx-overlay tx-inspector" aria-label="Inspect ${escapeHtml(destination.name)}"><header><div><span>${escapeHtml(destination.type)} · destination detail</span><h2>${escapeHtml(destination.name)}</h2></div><button type="button" data-action="tx-close-inspector" aria-label="Close inspection">${icon('close', 20)}</button></header><div class="tx-overlay-scroll"><p>${escapeHtml(destination.context)}. Each Message and Seed is checked against this destination independently.</p><dl>${rows.map(([term, value]) => `<div><dt>${term}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl><div class="tx-expression-note"><b>Destination boundary</b><p>Selection does not grant permission, transmit the Signal, prove delivery, or establish Semantic Landing.</p></div></div></aside>`;
  }
  const rows = [['Stable ID', seed.id], ['Creator', seed.creator], ['Context', seed.context], ['Privacy', seed.privacy], ['Status', seed.status], ['Version', seed.version], ['Eligibility', seed.eligibility], ['Provenance', seed.provenance], ['Rights / authority', seed.rights], ['Visual status', seed.visualStatus], ['Tags', seed.tags.join(', ')]];
  return `<aside class="tx-overlay tx-inspector" aria-label="Inspect ${escapeHtml(seed.title)}"><header><div><span>${escapeHtml(seed.type)} · full detail</span><h2>${escapeHtml(seed.title)}</h2></div><button type="button" data-action="tx-close-inspector" aria-label="Close inspection">${icon('close', 20)}</button></header><div class="tx-overlay-scroll"><p>${escapeHtml(seed.description)}</p><dl>${rows.map(([term, value]) => `<div><dt>${term}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl><div class="tx-expression-note"><b>Seed ≠ Expression</b><p>This is the identified Seed. It becomes an Expression only when it is opened, instantiated, rendered, experienced, or run.</p></div><button class="add" type="button" ${state.transmitterV2SeedIds.includes(seed.id) ? `data-tx-seed-remove="${seed.id}"` : `data-tx-seed-add="${seed.id}"`}>${state.transmitterV2SeedIds.includes(seed.id) ? 'Remove from Payload' : 'Add to Payload'}</button></div></aside>`;
}

function transmitterReview(state, result) {
  if (!state.transmitterV2ReviewOpen) return '';
  return `<aside class="tx-overlay tx-review" aria-label="Review Signal"><header><div><span>Final review · nothing sent</span><h2>Review the Signal</h2></div><button type="button" data-action="tx-close-review" aria-label="Close review">${icon('close', 20)}</button></header><div class="tx-overlay-scroll"><div class="tx-review-anatomy"><article><span>Message</span><p>${escapeHtml(state.transmitterV2Message || 'No Message yet.')}</p><small>${escapeHtml(state.transmitterV2MessagePrivacy)} boundary</small></article><i>+</i><article><span>Payload</span><p>${result.seeds.map(seed => escapeHtml(seed.title)).join(' · ') || 'No Seeds yet.'}</p><small>${result.seeds.length} identified Seed${result.seeds.length === 1 ? '' : 's'}</small></article></div><dl class="tx-review-summary"><div><dt>Payload name</dt><dd>${escapeHtml(state.transmitterV2PayloadName)}</dd></div><div><dt>Send as</dt><dd>${escapeHtml(result.sender.name)} · recipient sees ${escapeHtml(result.sender.recipientView)}</dd></div><div><dt>Destinations</dt><dd>${result.destinations.map(item => escapeHtml(item.name)).join(', ') || 'None selected'}</dd></div></dl>${transmitterPrivacyCheck(result)}<section class="tx-outcomes"><h3>Possible delivery outcomes</h3><p>Transmitted, landed, held, denied, failed, or awaiting approval remain distinct. Preparing this prototype creates none of those receipts.</p></section>${state.transmitterV2Prepared ? '<div class="tx-prepared" role="status"><b>Transmission prepared locally.</b><span>Nothing has been sent.</span></div>' : ''}<button class="tx-prepare" type="button" data-action="tx-prepare" ${result.ready ? '' : 'disabled'}>Prepare transmission</button><small class="tx-review-footnote">This static prototype cannot send, publish, grant access, move Resources, or create a delivery receipt.</small></div></aside>`;
}

function transmitterKi(state) {
  return `<section class="receiver-v2-ki tx-ki" aria-label="Work with Ki in Transmitter"><div class="ki-word-circle" aria-hidden="true">Ki</div><label><span>Ki · Transmitter</span><input id="tx-ki-input" type="text" value="${escapeHtml(state.transmitterV2KiDraft)}" placeholder="Find Seeds, suggest destinations, or propose changes to this Signal…"></label><div class="receiver-v2-voice-actions"><button type="button" data-action="tx-dictate"><span class="interface-icon icon-microphone" aria-hidden="true"></span><span>Dictate</span></button><button type="button" data-action="tx-voice"><span class="interface-icon icon-voice-live" aria-hidden="true"></span><span>Voice</span></button><button class="receiver-v2-preview" type="button" data-action="tx-ki-preview">Preview</button></div>${state.transmitterV2Proposal ? `<div class="receiver-v2-ki-proposal" role="status"><span>Review Ki’s proposal</span><ul>${state.transmitterV2Proposal.summary.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul><div><button class="add" type="button" data-action="tx-ki-apply">Apply</button><button type="button" data-action="tx-ki-revise">Revise</button><button type="button" data-action="tx-ki-cancel">Cancel</button></div></div>` : ''}</section>`;
}

function transmitPanelV2(state) {
  const query = state.transmitterV2Query.toLowerCase();
  const filters = new Set(state.transmitterV2Filters || []);
  const searchable = seed => [seed.title, seed.description, seed.type, seed.creator, seed.context, seed.tags, seed.privacy, seed.status].flat().join(' ').toLowerCase();
  let seeds = transmitterSeedCatalog.filter(seed => filters.has(seed.type) && (!query || searchable(seed).includes(query)));
  seeds = [...seeds].sort((a, b) => state.transmitterV2Sort === 'Oldest' ? new Date(a.createdAt) - new Date(b.createdAt) : state.transmitterV2Sort === 'Type' ? a.type.localeCompare(b.type) || a.title.localeCompare(b.title) : state.transmitterV2Sort === 'Privacy' ? privacyOrder[a.privacy] - privacyOrder[b.privacy] || a.title.localeCompare(b.title) : state.transmitterV2Sort === 'Status' ? a.status.localeCompare(b.status) || a.title.localeCompare(b.title) : state.transmitterV2Sort === 'Title' ? a.title.localeCompare(b.title) : new Date(b.createdAt) - new Date(a.createdAt));
  const result = evaluateTransmissionDraft(transmitterDraft(state));
  const allSelected = transmitterSeedTypes.every(type => filters.has(type));
  const eyebrow = ['What are we sending into the Field?', 'Your stuff, gathered with intention.', 'Make the boundary as clear as the message.'][Math.max(0, state.transmitterV2EyebrowIndex) % 3];
  return `<section class="receiver-v2 transmitter-v2 tx-step-${state.transmitterV2Step.toLowerCase()}" aria-label="Transmitter Version 0.02" data-open-panel="transmit"><div class="receiver-v2-atmosphere" aria-hidden="true"><i></i><i></i><i></i><i></i></div><header class="receiver-v2-topbar"><div class="receiver-v2-brand"><span class="rail-role-icon role-icon-transmitter" aria-hidden="true"></span><div><span>${escapeHtml(eyebrow)}</span><h1>Transmitter</h1></div></div><div class="tx-top-state"><span>Draft status</span><strong>${escapeHtml(result.status)}</strong></div><button class="receiver-v2-close transmitter-v2-close" type="button" data-close-panel aria-label="Close Transmitter">${icon('close', 22)}</button></header>
    <section class="receiver-v2-controls tx-controls"><label class="receiver-v2-search">${icon('search', 19)}<span class="sr-only">Search your Seed library</span><input id="tx-search" type="search" value="${escapeHtml(state.transmitterV2Query)}" placeholder="Search your Seeds by title, creator, Realm, Map, tag, privacy, or state…"></label><nav aria-label="Seed type filters"><button class="${allSelected ? 'active' : ''}" type="button" data-tx-filter="All" aria-pressed="${allSelected}">All</button>${transmitterSeedTypes.map(type => `<button class="${filters.has(type) ? 'active' : ''}" type="button" data-tx-filter="${type}" aria-pressed="${filters.has(type)}">${type === 'Other' ? type : `${type}s`}</button>`).join('')}</nav><label class="receiver-v2-sort"><span>Sort by</span><select id="tx-sort">${['Most recent', 'Oldest', 'Type', 'Privacy', 'Status', 'Title'].map(sort => `<option ${state.transmitterV2Sort === sort ? 'selected' : ''}>${sort}</option>`).join('')}</select></label></section>
    <nav class="tx-mobile-steps" aria-label="Transmission steps">${['Seeds', 'Signal', 'Destinations', 'Review'].map(step => `<button class="${state.transmitterV2Step === step ? 'active' : ''}" type="button" data-tx-step="${step}">${step}</button>`).join('')}</nav><div class="tx-workspace"><main class="tx-library"><header><div><span>${seeds.length} visible · ${state.transmitterV2SeedIds.length} selected</span><h2>Your Seed library</h2><p>Your stuff stays individually identified inside a Payload. It may contribute to an Expression when it is opened or presented.</p></div></header><div class="tx-seed-grid">${seeds.map(seed => transmitterSeedCard(state, seed)).join('') || '<div class="receiver-v2-empty"><h2>No visible Seeds</h2><p>Select at least one type or adjust your search.</p></div>'}</div></main>${transmitterBuilder(state, result)}</div>${transmitterInspector(state)}${transmitterReview(state, result)}</section>`;
}
...
function createPanel() {
  const groups = [
    ['Meaning', ['Map', 'Realm', 'Node', 'Media']],
    ['Agents', ['Avatar', 'Ally', 'Actor', 'Automation']],
    ['Expressions', ['Composed', 'Setting', 'Pulse', 'Graph', 'Theater', 'World', 'Reader', 'Game']],
...
function packagesPanel(state) {
  return panelShell('packages', 'Seeds', 'Bounded starting units', `
    <p class="panel-lede">Seeds are bounded collections or starting units. Open one without losing your place in the Field.</p>
    ...
    ${actionRow('Ask Ki to recommend a Seed')}`);
}
...
export function panelFor(state) {
  switch (state.openPanel) {
    ...
    case 'transmit': return state.receiverExperiment === '0.02' ? transmitPanelV2(state) : transmitPanel(state);
...
export function panelTitle(id) {
  return ({ vault: 'Moto’s Vault', receive: 'Receive', transmit: 'Transmit', connect: 'Connect', create: 'Create', packages: 'Seeds', history: 'History', settings: 'Settings', avatar: 'Active Avatar', type: 'Layout Type', map: 'Map', economics: 'Economics', governance: 'Governance', culture: 'Culture' })[id] || id;
}
